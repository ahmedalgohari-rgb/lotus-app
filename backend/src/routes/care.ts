...
const getAuthUser = (req: express.Request) => (req as AuthenticatedRequest).user!;
...
router.get('/recent', authMiddleware, async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    const userId = getAuthUser(req).id;
...
router.get('/stats', authMiddleware, async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
    const userId = getAuthUser(req).id;
